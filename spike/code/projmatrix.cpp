//[header]
// A simple program to demonstrate how to build and use a simple perspective
// projection matrix
//[/header]
//[compile]
// Download the projmatrix.cpp, vertexdata.h and geometry.h files to the same
// folder.
...
//[/comment]
void setProjectionMatrix(const float &angleOfView, const float &near,
                         const float &far, Matrix44f &M) {
    // do some work here
...
// coordinates but because we will multiply this point by a 4x4 matrix we
// actually
// assume it is a point with homogeneous coordinatess (x, y, z, w = 1).
// The point-matrix results in another point with homogeneous coordinates (x',
// y', z', w').
// To get back to Cartesian coordinates we need to noramlized these coordinates:
// (x'/w', y'/w', z'/w').
//[/comment]
void multPointMatrix(const Vec3f &in, Vec3f &out, const Matrix44f &M) {
    ...
    out.z = in.x * M[0][2] + in.y * M[1][2] + in.z * M[2][2] +
            /* in.z = 1 */ M[3][2];
    float w = in.x * M[0][3] + in.y * M[1][3] + in.z * M[2][3] +
              /* in.z = 1 */ M[3][3];
...
int main(int argc, char **argv) {
    ...
    for (uint32_t i = 0; i < numVertices; ++i) {
        ...
        if (projectedVert.x < -1 || projectedVert.x > 1 ||
            projectedVert.y < -1 || projectedVert.y > 1)
            continue;
        // convert to raster space and mark the position of the vertex in the
        // image with a simple dot
        uint32_t x = std::min(imageWidth - 1, (uint32_t)((projectedVert.x + 1) *
                                                         0.5 * imageWidth));
        uint32_t y = std::min(
            imageHeight - 1,
            (uint32_t)((1 - (projectedVert.y + 1) * 0.5) * imageHeight));
