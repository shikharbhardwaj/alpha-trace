...
// Open a shell/terminal, and run the following command where the files are
// saved:
//
// c++ pinhole.cpp -o pinhole -std=c++11
//
// Run with: ./pinhole. Open the file ./pinhole.svg in any Internet browser to
// see
// the result.
//[/compile]
//[ignore]
...
//[comment]
// Compute the 2D pixel coordinates of a point defined in world space. This
// function
// requires the point original world coordinates of course, the world-to-camera
// matrix (which you can get from computing the inverse of the camera-to-world
// matrix,
// the matrix transforming the camera), the canvas dimension and the image width
// and
// height in pixels.
//
// The canvas coordinates (bottom, left, top, right) are also passed to the
// function
// so we can test the point raster coordinates against the canvas coordinates.
// If the
// point in raster space lies within the canvas boundaries, the function returns
// true,
// false otherwise.
//[/comment]
bool computePixelCoordinates(const Vec3f &pWorld,
                             const Matrix44f &worldToCamera, const float &b,
                             const float &l, const float &t, const float &r,
                             const float &near, const uint32_t &imageWidth,
                             const uint32_t &imageHeight, Vec2i &pRaster) {
    Vec3f pCamera;
...
int main() {
    //[comment]
    // First compute the canvas coordinates. The canvas is positionned by
    // choice,
    // at the near clipping plane. By changing the near clipping plane value,
    // you
    // will change the position of the canvas, but this won't change the output
    // of the
    // program.
    //[/comment]
    ...
    float top =
        ((filmApertureHeight * inchToMm / 2) / focalLength) * nearClippingPlane;
    float right =
        ((filmApertureWidth * inchToMm / 2) / focalLength) * nearClippingPlane;
    ...
    printf("Angle of view: %f (deg)\n",
           2 * atan((filmApertureWidth * inchToMm / 2) / focalLength) * 180 /
               M_PI);

    //[comment]
    // Project the triangles vertices onto the plane. If at least one of the
    // vertices lies outside
    // the canvas boundaries (if the function computePixelCoordinates returns
    // false), the triangle
    ...
    ofs << "<svg version=\"1.1\" xmlns:xlink=\"http://www.w3.org/1999/xlink\" "
           "xmlns=\"http://www.w3.org/2000/svg\" width=\"" << imageWidth
        << "\" height=\"" << imageHeight << "\">" << std::endl;
    Matrix44f cameraToWorld(-0.95424, 0, 0.299041, 0, 0.0861242, 0.95763,
                            0.274823, 0, -0.28637, 0.288002, -0.913809, 0,
    ...
    for (uint32_t i = 0; i < numTris; ++i) {
        ...
        bool visible = true;
        visible &= computePixelCoordinates(v0World, worldToCamera, bottom, left,
                                           top, right, nearClippingPlane,
                                           imageWidth, imageHeight, v0Raster);
        visible &= computePixelCoordinates(v1World, worldToCamera, bottom, left,
                                           top, right, nearClippingPlane,
                                           imageWidth, imageHeight, v1Raster);
        visible &= computePixelCoordinates(v2World, worldToCamera, bottom, left,
                                           top, right, nearClippingPlane,
                                           imageWidth, imageHeight, v2Raster);

        int val = visible ? 0 : 255;
        ofs << "<line x1=\"" << v0Raster.x << "\" y1=\"" << v0Raster.y
            << "\" x2=\"" << v1Raster.x << "\" y2=\"" << v1Raster.y
            << "\" style=\"stroke:rgb(" << val << ",0,0);stroke-width:1\" />\n";
        ofs << "<line x1=\"" << v1Raster.x << "\" y1=\"" << v1Raster.y
            << "\" x2=\"" << v2Raster.x << "\" y2=\"" << v2Raster.y
            << "\" style=\"stroke:rgb(" << val << ",0,0);stroke-width:1\" />\n";
        ofs << "<line x1=\"" << v2Raster.x << "\" y1=\"" << v2Raster.y
            << "\" x2=\"" << v0Raster.x << "\" y2=\"" << v0Raster.y
            << "\" style=\"stroke:rgb(" << val << ",0,0);stroke-width:1\" />\n";
    }
    ofs << "</svg>\n";
    ofs.close();
