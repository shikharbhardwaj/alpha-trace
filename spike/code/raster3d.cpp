...
void computeScreenCoordinates(const float &filmApertureWidth,
                              ...
                              const float &focalLength, float &top,
    ...
    float fov =
        2 * 180 / M_PI * atan((filmApertureWidth * inchToMm / 2) / focalLength);
...
//[/comment]
void convertToRaster(const Vec3f &vertexWorld, const Matrix44f &worldToCamera,
                     const float &l, const float &r, const float &t,
                     const float &b, const float &near,
...
int main() {
    ...
    computeScreenCoordinates(filmApertureWidth, filmApertureHeight, imageWidth,
                             imageHeight, kOverscan, nearClippingPLane,
                             focalLength, t, b, l, r);
    ...
    for (uint32_t i = 0; i < ntris; ++i) {
        ...
        convertToRaster(v0, worldToCamera, l, r, t, b, nearClippingPLane,
                        imageWidth, imageHeight, v0Raster);
        convertToRaster(v1, worldToCamera, l, r, t, b, nearClippingPLane,
        ...
        // [/comment]
        v0Raster.z = 1 / v0Raster.z, v1Raster.z = 1 / v1Raster.z,
        ...
        if (xmin > imageWidth - 1 || xmax < 0 || ymin > imageHeight - 1 ||
            ymax < 0)
            continue;
        ...
        uint32_t x1 =
            std::min(int32_t(imageWidth) - 1, (int32_t)(std::floor(xmax)));
        ...
        for (uint32_t y = y0; y <= y1; ++y) {
            for (uint32_t x = x0; x <= x1; ++x) {
                ...
                if (w0 >= 0 && w1 >= 0 && w2 >= 0) {
                    ...
                    w2 /= area;
                    float oneOverZ =
                        v0Raster.z * w0 + v1Raster.z * w1 + v2Raster.z * w2;
                    float z = 1 / oneOverZ;
                    // [comment]
                    // Depth-buffer test
                    // [/comment]
                    if (z < depthBuffer[y * imageWidth + x]) {
                        ...
                        // [comment]
                        // If you need to compute the actual position of the
                        // shaded
                        // point in camera space. Proceed like with the other
                        // vertex attribute.
                        // Divide the point coordinates by the vertex
                        // z-coordinate then
                        // interpolate using barycentric coordinates and finally
                        // multiply
                        // by sample depth.
                        // [/comment]
                        ...
                        float px = (v0Cam.x / -v0Cam.z) * w0 +
                                   (v1Cam.x / -v1Cam.z) * w1 +
                                   (v2Cam.x / -v2Cam.z) * w2;
                        float py = (v0Cam.y / -v0Cam.z) * w0 +
                                   (v1Cam.y / -v1Cam.z) * w1 +
                                   (v2Cam.y / -v2Cam.z) * w2;
                        ...
                        // [comment]
                        // Compute the face normal which is used for a simple
                        // facing ratio.
                        // Keep in mind that we are doing all calculation in
                        // camera space.
                        // Thus the view direction can be computed as the point
                        // on the object
                        // in camera space minus Vec3f(0), the position of the
                        // camera in camera
                        // space.
                        // [/comment]
                        ...
                        float nDotView =
                            std::max(0.f, n.dotProduct(viewDirection));

                        // [comment]
                        // The final color is the reuslt of the faction ration
                        // multiplied by the
    ...
    // [comment]
    // Store the result of the framebuffer to a PPM file (Photoshop reads PPM
    // files).
    // [/comment]
    std::ofstream ofs;
    ofs.open("./output.ppm");
