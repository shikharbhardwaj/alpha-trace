//[header]
// This program renders a wireframe image of a 3D object whose description is
// stored
// in the program itself. The result is stored to a SVG file. To draw an image
// of each triangle making up that object, we project the vertices of each
// triangle
// onto the screen using perspective projection (effectively transforming the
// vertices
...
//
// Run with: ./perspproj. Open the file ./proj.svg in any Internet browser to
// see
// the result.
//[/compile]
//[ignore]
...
//[/comment]
const Vec3f verts[146] = {{0, 39.034, 0},
                          {0.76212, 36.843, 0},
                          {3, 36.604, 0},
                          {1, 35.604, 0},
                          {2.0162, 33.382, 0},
...
// height in pixels.
//
// Note that we don't check in this version of the function if the point is
// visible
...
// When a SVG file is displayed to the screen, the application displaying the
// content
// of the file clips points and lines which are not visible. Thus we can store
// lines or point
// in the file whether they are visible or not. The final clipping will be done
// when the
// image is displayed to the screen.
//[/comment]
//
void computePixelCoordinates(const Vec3f &p_world, Vec2i &p_raster,
                             const Matrix44f &world_to_cam,
    ...
    world_to_cam.multVecMatrix(p_world, p_cam);
...
int main() {
    ...
    //[/comment]
    Matrix44f cameraToWorld(0.871214, 0, -0.490904, 0, -0.192902, 0.919559,
                            -0.342346, 0, 0.451415, 0.392953, 0.801132, 0,
    ...
    for (uint32_t i = 0; i < numTris; ++i) {
        ...
        computePixelCoordinates(v2World, v2Raster, worldToCamera, canvasWidth,
                                canvasHeight, imageWidth, imageHeight);
        std::cerr << v0Raster << ", " << v1Raster << ", " << v2Raster
                  << std::endl;
