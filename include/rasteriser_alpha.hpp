...
class Rasteriser {
  ...
  public:
    ...
    void draw_triangle_16xAA(const Point &v0, const Point &v1, const Point &v2,
                             Vec2f &st0, Vec2f &st1, Vec2f &st2) {
        // TODO: Optimize
        Vec3f v0_rast, v1_rast, v2_rast;
        cam->convert_to_raster(v0, v0_rast);
        ...
        for (uint32_t y = y0; y <= y1; y++) {
            ...
            for (uint32_t x = x0; x <= x1; x++) {
                ...
                if (w0 >= 0 && w1 >= 0 && w2 >= 0) {
                    ...
                    if (z < Zbuf->get(x, y)) {
                        Zbuf->set(x, y, z);
                        std::vector<Vec3f> pixel_samples(16);
                        ...
                        for (auto elem : pixel_samples) {
                            ...
                            alpha::math::Vec3f pt(px * z, py * z,
                                                  -z); // In camera space
                            auto normal = (v1_cam - v0_cam)
                                              .cross_product(v2_cam - v0_cam);
                            normal.normalize();
                            Vec3f view_dir = pt * -1.f;
                            view_dir.normalize();
                            float n_dot_alpha =
                                std::max(0.f, normal.dot_product(view_dir));
                            // This is the value of "intensity" ratio, which is
                            // to
                            // be multiplied with the original color at the
                            // point to
                            // obtain the final shade, when viewed at the
                            // current
                            // perspective
                            ...
                            float checker = (fmod(st.x * M, 1.0) > 0.5) ^
                                            (fmod(st.y * M, 1.0) < 0.5);
                            float c = 0.3 * (1 - checker) + 0.7 * checker;
                            total_color.x += 255 * n_dot_alpha * c;
                            total_color.y += 255 * n_dot_alpha * c;
                            total_color.z += 255 * n_dot_alpha * c;
                        }
